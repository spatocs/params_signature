...
=head1 EXTENDED DISCUSSION

This manual is the long-form guide for Params::Signature.  The module POD
(L<Params::Signature>) is the canonical API reference; this manual focuses on
background, usage patterns, tradeoffs, and detailed examples.

Params::Signature provides signature-based validation intended to stay concise
at call sites while still allowing strong type checks, default values,
callbacks, aliases, and mixed positional/named calling conventions.

The module works with type constraints from the caller's namespace (for
example Types::Standard/Type::Tiny declarations), Moose/Mouse constraints, and
plain code references.  This lets you keep validation close to your domain
types instead of hard-coding ad-hoc validation logic in each subroutine.

For production code, use one validator object per module/application and reuse
it across calls.  This improves consistency and takes advantage of signature
caching.

=head2 API Quick Reference
...
Helper constants exported via C<:all>:
...
In its simplest form, you simply call Params::Signature's validate method with your subroutine's parameters and a signature specification:
    ...
    $params = validate_method(\@_, ["Object self", "Str x = 'default'", "Undef|Str y?"]);

Once C<validate_method> validates the first parameter (self), the first parameter is removed from the argument list passed to L<Params::Signature/validate>.  As a result, documentation describing C<validate> applies equally to C<validate_method>.

The signature is a list of parameter definitions.  A basic parameter definition is a string which consists of a type constraint and the name of the parameter.  The actual type constraints are defined via an external module, such as L<Type::Tiny> or L<Moose::Util::TypeConstraints>.  A subroutine reference may also be used as a type constraint.  Parameters are required by default.  A default value may be assigned to a required parameter, but not an optional one.  Parameters may be flagged as optional using the optional flag (trailing question mark).
...
Params::Signature can be used in one of three different styles - class methods, object methods or exported subroutines.  The methods C<validate>, C<validate_method>  and C<check> can be called as class methods.  The global default settings are used with class methods.  A Params::Signature object may be used to set custom settings.  A singleton per module/application should reduce the need to configure settings for each call to C<validate>.   The C<validate>  C<validate_method> and C<check> class methods can be exported and used as regular subroutines.  Under the covers, these calls are treated as calls to class methods, which means default settings are used unless overriden.  When using the exported subroutines, a hash passed to the C<validate> subroutine can be used to override global settings.
...
The examples below attempt to demonstrate each of the usage styles along with various parameter styles, which are discussed in the next section.
    ...
    #   call to validate
    my ($self, $number) = validate_method(
            \@_,
    ...
    my ($self, $one, $two) = validate_method(\@_, ["Object self", "Str one", "Str two"]);

    my $params = validate_method(\@_, ["Object self", "Str one", "Str two"]);
...
If "fuzzy" is enabled, the L<Params::Signature/validate> method will attempt to detect the parameter style by examining the parameters.  In most cases, it will work as expected (provided you expect the right thing).  The way to distinguish between positional and named parameters is to pass named parameters inside an anonymous hash.

B<How it works>

When "fuzzy" is set to "1", L<Params::Signature/validate> detects if one parameter, a hash, is passed in as the first and only parameter.  If so, the hash's keys are examined to determine if they match any parameter names.  If any key matches any parameter name, the contents of the hash are validated.

When fuzzy is set to "2", the rules for "fuzzy=1" are applied first.  If that fails and C<@_> contains an even-sized list of values, the values are examined to see if the values alternate between a parameter name and a value.  If a parameter name is found in an even element (index 0, 2, 4, etc.), the "named" parameter style is used.  If no parameter names are found where they should be, the "named" style is not used.  If the "named" style cannot be detected, L<Params::Signature/validate> will use the default style in effect (which should be "positional").
...
The extra ('C<...>') pseudo-parameter is used to indicate that extra parameters are allowed.  Extra parameters are not validated, they are simply appended to the list returned by the L<Params::Signature/validate> method.  If a signature ends with named parameters, then all extra parameters must also be named.  A 'best effort' attempt is made to ensure the extra parameters are named.  If a signature ends with positional parameters, all extra parameters are treated as positional as well. Extra positional parameters are assigned unique names in the hash returned by C<validate>.
...
Params::Signature::Multi provides signature-based subroutine dispatch.  You
define multiple candidate subroutines, each with its own signature, and the
multi dispatcher calls the first subroutine whose signature matches the
arguments.

A Params::Signature::Multi object holds a L<Params::Signature> validator that
it uses to check each candidate signature against the arguments.  The
validator can be replaced with C<< $multi->validator($new_validator) >> to
customize parameter style, fuzzy mode, coercion, or any other validator
option.

=head2 Basic dispatch
...
The signatures are tried in order.  The first signature whose types accept all
arguments is selected and its C<call> subroutine is invoked.  Default values
in the signatures are I<ignored> during dispatch -- only type constraints are
checked.

=head2 Resolving without calling

C<resolve> finds the matching signature but does not call anyone.  It returns
the index and optional C<id> of the matched signature, plus the validated
parameters:
...
=head2 Signature ordering

Because the dispatcher stops at the first match, signatures should be ordered
from most specific to least specific.  An C<Int> constraint is stricter than
a C<Str> constraint, so signatures with C<Int> parameters should appear
before signatures with C<Str> parameters that accept the same number of
arguments:
...
=head2 Fallback signature

A signature of C<['...']> matches anything and is commonly placed last so
that unmatched arguments are always handled (even if only to produce an error
message).

=head2 Using named parameters with dispatch

The parameter style can be set on the Multi object's validator or passed per
signature via the C<config> hash:
...
For more details see L<Params::Signature::Multi>.

=head1 SEE ALSO

L<Params::Signature::Multi>, L<Params::Validate>, L<MooseX::Method::Signatures>, L<Method::Signatures>, L<Perl6::Signature>
