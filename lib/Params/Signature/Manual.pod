...
Once C<validate_method> validates the first parameter (self), the first parameter is removed from the argument list passed to C</validate>.  As a result, documentation describing C<validate> applies equally to C<validate_method>.
