...
Main:
{
    ...
    diag("Test custom type constraint coercion via exported subroutines, Perl $], $^X");
...
sub test_coerce_hash_ref_to_params_signature
{
    ...
    register_coerce("HashRef", "Params::Signature", sub { new Params::Signature(%{$_[0]})});

    $answer = coerce(to => "Params::Signature",
...
sub test_coerce_hash_ref_to_params_signature_many_types
{
    ...
    $answer = coerce(to => "Str|Int|Params::Signature",
                    from => { param_style => 'named', register_builtins => 0, called => 'testXYZ'},
                    info => $info
        );
    $failed_msg = $info->{msg};
...
sub test_coerce_already_one_of_to_type
{
    ...
    $answer = coerce(to => "Str|Num|Class|Params::Signature",
                    from => $signature,
                    info => $info
        );
