...
my $test_count = 0;
my $object = bless({}, "SomeClass");

my @test_criteria = (
    {
     name         => "Pos: 3 required",
     args         => [$object,1, 2, 3],
     signature    => ["Object self","Int one", "Int two", "Int three"],
     ok           => 1,
     array_answer => [$object,1, 2, 3]
    },
    {
     ...
     signature => ["Object self","Int one", "Int two", "Int three"],
     ok        => 0,
     array_answer => [$object,1, 2,]
    },
    {name => "Pos: 2 optional set", args => [$object,1, 2,], signature => ["Object self","Int one?", "Int two?",], ok => 1, array_answer => [$object,1, 2,]},
    {name => "Pos: 0 of 2 optional set", args => [$object],  signature => ["Object self","Int one?", "Int two?",], ok => 1},
    {name => "Pos: 1 of 2 optional set", args => [$object,1], signature => ["Object self","Int one?", "Int two?",], ok => 1},
    {
     name      => "Pos: 3 required, not set, with default",
     args      => [$object],
     signature => ["Object self","Int one = 1", "Int two = 2", "Int three = 3"],
     ok        => 1
    },

    {name => "Pos: 3 required, not set, no default", args => [$object], signature => ["Object self","Int one", "Int two", "Int three"], ok => 0},
    {
     name      => "Pos: 3 required, not set, invalid default",
     args      => [$object],
     signature => ["Object self","Int one = 'one'", "Int two = 'two'", "Int three = 'three'"],
     ok        => 0
    },
    {
     name         => "Pos: 3 optional, 1 set, 2 depends fields with defaults",
     args         => [$object,1],
     signature    => ["Object self","Int one? << [two, three]", "Int two? = 2", "Int three? = 3"],
     ok           => 1,
     array_answer => [$object,1, 2, 3]
    },
    {
     ...
     args      => [$object,1],
     signature => ["Object self","optional:", "Int one << [two, three]", "Int two = 2", "Int three = 3"],
     ok        => 1,
     array_answer => [$object,1, 2, 3]
    },

    {
     ...
     args         => [$object,1],
     signature    => ["Object self","Int one? << [three]", "Int two? = 2", "Int three? = 3"],
     ok           => 1,
     array_answer => [$object,1, undef, 3]
    },
    {
     ...
     ok           => 1,
     array_answer => [$object],
     hash_answer  => {self => $object}
    },
    {
     name         => "Pos: 3 optional, 0 set, 1 depends field with default, optional: set",
     args         => [$object],
     signature    => ["Object self","optional:", "Int one << [three]", "Int two = 2", "Int three = 3"],
     ok           => 1,
     array_answer => [$object],
     hash_answer  => {self => $object}
    },

    {
     ...
     args         => [$object],
     signature    => ["Object self","Int one? << [three, two]", "Int two? = 2", "Int three? = 3"],
     ok           => 1,
     array_answer => [$object],
     hash_answer  => {self => $object}
    },
    {
     ...
     args         => [$object,1],
     signature    => ["Object self","Int one", "Int two <= one"],
     ok           => 1,
     array_answer => [$object,1, 1],
     hash_answer  => {self => $object}
    },
    {
    ...
    },
    {
     ...
     args         => [$object],
     signature    => ["Object self","Int one = { 4*1 }"],
     ok           => 1,
     array_answer => [$object,4],
     hash_answer  => {self => $object, one => 4}
    },
    {
     name        => "Pos: 1 required, 0 set, 1 with sub default",
     args        => [$object],
    ...
    },
    {
     ...
     args        => [$object,1],
     signature   => ["Object self","Int one", "Int two? = { 2*2 }"],
     ok          => 1,
     hash_answer => {self => $object, one => 1}
    },
    {
     name         => "Pos: 1 required, 2 optional, 1 set, 1 with sub default",
     args         => [$object,1],
     signature    => ["Object self","Int one", "Int two? = { 2*2 }", "Int three?"],
     ok           => 1,
     array_answer => [$object,1]
    },
    {
     name         => "Pos: 1 required, 1 extra with extra",
     args         => [$object,1, 2],
     signature    => ["Object self","Int one", "..."],
     ok           => 1,
     array_answer => [$object,1, 2]
    },
    {name => "Pos: 0 required, 2 extra with extra", args => [$object,1, 2], signature => ["Object self","..."], ok => 1, array_answer => [$object,1, 2]},
    {
     name         => "Pos: 3 optional, 1 set, 2 depends fields with defaults, use alias",
     args         => [$object,1],
     signature    => ["Object self","Int one|uno? << [dos, tres]", "Int two|dos? = 2", "Int three|tres? = 3"],
     ok           => 1,
     array_answer => [$object,1, 2, 3]
    },
    {
...

    {
     ...
     args         => [$object,1, {two => 2}],
     signature    => ["Object self","Int one", "named:", "Int two"],
     ok           => 1,
     array_answer => [$object,1, 2]
    },
    {
     ...
     signature   => ["Object self","Int one", "named:", "Int two"],
     ok          => 1,
     hash_answer => {self => $object, one => 1, two => 2}
    },
    {
     name        => "Mixed: 3 required, 3 set, 2 pos, 1 named, no default",
     args        => [$object,1, 2, {three => 3}],
     signature   => ["Object self","Int one", "Int two", "named:", "Int three"],
     ok          => 1,
     hash_answer => {self => $object, one => 1, two => 2, three => 3}
    },
    {
     name         => "Mixed: 3 required, 3 set, 2 pos, 1 named, no default",
     args         => [$object,1, 2, {three => 3}],
    ...
    },
    {
     ...
     args => [$object,1, 2, {three => 3, four => 4}],
     signature => ["Object self","Int one", "Int two", "named:", "Int three", "Int four"],
     ok        => 1,
     hash_answer => {self => $object, one => 1, two => 2, three => 3, four => 4}
    },
    {
     ...
     signature => ["Object self","Int one", "Int two", "named:", "Int three", "Int four"],
     ok        => 1,
     array_answer => [$object,1, 2, 3, 4]
    },
    {
     name => "Mixed: 4 required, 4 set, 2 pos, 2 named in hash, no default, use ':param_name'",
     args => [$object,1, 2, {three => 3, four => 4}],
     signature => ["Object self","Int one", "Int two", "Int :three", "Int :four"],
     ok        => 1,
     array_answer => [$object,1, 2, 3, 4]
    },
    {
    ...
    },
    {
     ...
     args         => [$object,1, {dos => 2}],
     signature    => ["Object self","Int one", "named:", "Int two|dos"],
     ok           => 1,
     array_answer => [$object,1, 2]
    },
    {
...

    {
     ...
     args => {one => 1, two => 2,},
     signature => ["Object self","named:", "Int one?", "Int two?",],
     ok        => 0
    },

    {
     ...
     args      => [$object,1, 2, 3, 4],
     signature => ["Object self","named:", "Int one", "Int two", "Int three", "Int four"],
     ok        => 0,
     msg_regex => "unexpected named parameter"
    },
    {
     name => "Named: 3 required, as key/value pairs",
     args => [$object,one => 1, two => 2, three => 3],
    ...
    },
    {
     ...
     args => [$object,{uno => 1, dos => 2, tres => 3}],
     signature => ["Object self","named:", "Int one|uno", "Int two|dos", "Int three|tres"],
     ok        => 1,
     hash_answer => {self => $object, one => 1, two => 2, three => 3}
    },
    ...
    {name => "Named: 0 of 2 optional set", args => [$object], signature => ["Object self","named:", "Int one?", "Int two?",], ok => 1},
    {name => "Named: 1 of 2 optional set, as hash", args => [$object,{two => 2}], signature => ["Object self","named:", "Int one?", "Int two?",], ok => 1},

    {name => "Named: 1 of 2 optional set, as key/value pairs", args => [$object,two => 2], signature => ["Object self","named:", "Int one?", "Int two?",], ok => 1},

    {
     name      => "Named: 3 required, not set, with default",
     args      => [$object],
     signature => ["Object self","named:", "Int one = 1", "Int two = 2", "Int three = 3"],
     ok        => 1
    },
    {
     name      => "Named: 3 required, not set, with default",
    ...
    },
    {
     ...
     args      => [$object],
     signature => ["Object self","named:", "Int one = 'one'", "Int two = 'two'", "Int three = 'three'"],
     ok        => 0,
    ...
    },
    {
     ...
     args      => [$object,{one => 1}],
     signature => ["Object self","named:", "Int one? << [two, three]", "Int two? = 2", "Int three? = 3"],
     ok        => 1
    },
    {
     name        => "Named: 3 optional, 1 set, 1 depends field with default",
     args        => [$object,{one => 1}],
     signature   => ["Object self","named:", "Int one? << [three]", "Int two? = 2", "Int three? = 3"],
     ok          => 1,
     hash_answer => {self => $object, one => 1, three => 3}
    },
    {
    ...
    },
    {
     ...
     args        => [$object],
     signature   => ["Object self","named:", "Int one? << [three, two]", "Int two? = 2", "Int three? = 3"],
     ok          => 1,
     hash_answer  => {self => $object}
    },
    {
     ...
     args        => [$object,{one => 1}],
     signature   => ["Object self","named:", "Int one", "Int two <= one"],
     ok          => 1,
     hash_answer => {self => $object, one => 1, two => 1}
    },
    {
    ...
    },
    {
     ...
     args        => [$object],
     signature   => ["Object self","named:", "Int one = { 2*2 }"],
     ok          => 1,
     hash_answer => {self => $object, one => 4}
    },
    {
     ...
     args      => [$object,{one => 1}],
     signature => ["Object self","named:", "Int one", "Int two? = { 2*2 }"],
     ok        => 1,
     hash_answer => {self => $object, one => 1}
    },
    {
...
my @test_fuzzy_criteria = (
    ...
    },
    {
     name => "fuzzy Named: 2 required, 1 optional, required present in array",
     args => [$object,one => 1, three => 3],
     signature => ["Object self","Str one", "Str two", "Str three?"],
     ok        => 0,
     msg_regex => "unexpected extra parameter"
    },

    {
     ...
     args        => [$object,"one", 2, "three", 4],
     signature   => ["Object self","Str one", "Str two?", "Str three?", "Str four?"],
     ok          => 0,
     hash_answer => {self => $object, one => 2, three => 4}
    },
    {
...

    {
     ...
     args        => [$object,"one", 'hey', 'hi'],
     signature   => ["Object self","Str one?", "Str two?", "..."],
     ok          => 1,
     hash_answer => {self => $object, one => 'one', two => 'hey', p_2 => "hi"}
    },
    {
     ...
     signature    => ["Object self","Str one?", "Str two?", "..."],
     ok           => 1,
     array_answer => [$object,'one', 'hey', 'hi']
    },
    ...
    },
    {
     ...
     args        => [$object,two => 2],
     signature   => ["Object self","Int one?", "Int two?"],
     ok          => 0,
    },
    {
     name        => "fuzzy Named: 1 optional, 1 present in hash",
    ...
    },
    {
     ...
     args => [$object,{two => 2, is_ok => 3}],
     signature => ["Object self","Int one?", "Int two?", "..."],
     ok        => 1,
     hash_answer => {self => $object, is_ok => 3, two => 2}
    },
...
my @test_fuzzy2_criteria = (
    ...
    {name => "Fuzzy 2 Named: 1 of 2 optional set, as key/value pairs", args => [$object,two => 2], signature => ["Object self","named:", "Int one?", "Int two?",], ok => 1},
    {
...
sub process_test_criteria
{
    ...
    foreach $criteria (@local_test_criteria)
    {
        $failed     = 0;
        $failed_msg = "";
        diag($criteria->{name}); # DEBUG

        if ($criteria->{array_answer})
        {
            @array_answer = $signature->validate_method($criteria->{args}, $criteria->{signature});
            ...
            if (!$failed)
            {
                foreach $idx (0 .. (scalar @{$criteria->{array_answer}} - 1))
                {
                    ...
                    if (ref($criteria->{array_answer}[$idx]) &&
                       ($criteria->{array_answer}[$idx] != $array_answer[$idx]))
                    {
                        $failed = 1;
                        $failed_msg =
        ...
        {
            $answer = $signature->validate_method($criteria->{args}, $criteria->{signature});
            ...
            if (!$failed)
            {
                foreach $key (keys %{$criteria->{hash_answer}})
                {
                    ...
                    if ($key eq "self")
                    {
                       if ($criteria->{hash_answer}{$key} != $answer->{$key})
                       {
                           $failed = 1;
                           $failed_msg =
            ...
            # this modifies (resets) the original hash in criteria->args so test can be re-used
            #delete $answer->{self};
        }
