...
my $signature = new Params::Signature(on_fail => sub { }, register_builtins => 1);
...
Main:
{
    ...
    foreach $type_test (@type_tests)
    {
        ...
        $answer    = $signature->type_check($type_name, $test_arg->{value});

        # some tests use undef as a value, so turn off warnings
        no warnings;
        ok($answer == $test_arg->{ok}, "$type_name: check '$test_arg->{value}'");
    }
